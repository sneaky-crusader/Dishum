...
## Returns true on the tick the punch becomes ACTIVE (i.e. the hit should be
## resolved by the caller against the opponent).
func advance(current_tick: int) -> bool:
	...
	if punch_phase == Combat.PunchPhase.WINDUP and elapsed >= Combat.WINDUP_TICKS:
		...
		return true
	elif punch_phase == Combat.PunchPhase.ACTIVE and elapsed >= Combat.WINDUP_TICKS + Combat.ACTIVE_TICKS:
		punch_phase = Combat.PunchPhase.RECOVERY
	elif punch_phase == Combat.PunchPhase.RECOVERY and elapsed >= Combat.WINDUP_TICKS + Combat.ACTIVE_TICKS + Combat.RECOVERY_TICKS:
